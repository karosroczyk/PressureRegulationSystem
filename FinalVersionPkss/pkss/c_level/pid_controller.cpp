...
PidController::PidController(float sumi) : m_sumI(sumi) {}
...
void PidController::UpdatePIDclass(std::string filename) {

	fstream fin;
	fin.open(filename, ios::in);

	string line, word, temp;
	PidController::PID.clear();
	while (fin >> temp) {
		getline(fin, line);
		stringstream s(line);
		while (getline(s, word, ',')) {
			PidController::PID.push_back(stof(word));
		}
	}
}
